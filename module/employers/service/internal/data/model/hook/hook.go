...
// The EmployersFunc type is an adapter to allow the use of ordinary
// function as Employers mutator.
type EmployersFunc func(context.Context, *model.EmployersMutation) (model.Value, error)

// Mutate calls f(ctx, m).
func (f EmployersFunc) Mutate(ctx context.Context, m model.Mutation) (model.Value, error) {
	if mv, ok := m.(*model.EmployersMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *model.EmployersMutation", m)
}
