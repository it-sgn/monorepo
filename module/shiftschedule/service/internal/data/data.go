...
import (
	...
	"mall-go/module/shiftschedule/service/internal/data/model"
...
var ProviderSet = wire.NewSet(NewData, NewEntClient, NewShiftScheduleRepo, NewRedisClient)
...
func NewData(entClient *model.Client, logger log.Logger, r registry.Discovery) (*Data, func(), error) {
	...
	if err != nil {
		logHelper.Fatalf("failed to connect to Employers service: %v", err)
	}
	...
	return &Data{
		db:               entClient,
		log:              logHelper,
		EmployersClient:  employerV1.NewEmployersClient(connEmp),
