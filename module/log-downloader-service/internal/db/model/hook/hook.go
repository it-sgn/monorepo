...
// The AttendanceLogFunc type is an adapter to allow the use of ordinary
// function as AttendanceLog mutator.
type AttendanceLogFunc func(context.Context, *model.AttendanceLogMutation) (model.Value, error)

// Mutate calls f(ctx, m).
func (f AttendanceLogFunc) Mutate(ctx context.Context, m model.Mutation) (model.Value, error) {
	if mv, ok := m.(*model.AttendanceLogMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *model.AttendanceLogMutation", m)
}
