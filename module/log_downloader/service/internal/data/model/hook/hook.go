...
// The CronZKFunc type is an adapter to allow the use of ordinary
// function as CronZK mutator.
type CronZKFunc func(context.Context, *model.CronZKMutation) (model.Value, error)

// Mutate calls f(ctx, m).
func (f CronZKFunc) Mutate(ctx context.Context, m model.Mutation) (model.Value, error) {
	if mv, ok := m.(*model.CronZKMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *model.CronZKMutation", m)
}
