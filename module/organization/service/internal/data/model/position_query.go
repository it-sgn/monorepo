...
func (_q *PositionQuery) QueryAssignments() *AssignmentQuery {
...
func (_q *PositionQuery) WithAssignments(opts ...func(*AssignmentQuery)) *PositionQuery {
	query := (&AssignmentClient{config: _q.config}).Query()
...
func (_q *PositionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Position, error) {
	...
	if query := _q.withAssignments; query != nil {
		if err := _q.loadAssignments(ctx, query, nodes,
			func(n *Position) { n.Edges.Assignments = []*Assignment{} },
			func(n *Position, e *Assignment) { n.Edges.Assignments = append(n.Edges.Assignments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PositionQuery) loadAssignments(ctx context.Context, query *AssignmentQuery, nodes []*Position, init func(*Position), assign func(*Position, *Assignment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(assignment.FieldPositionID)
	}
