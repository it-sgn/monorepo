...
// The BiometricFunc type is an adapter to allow the use of ordinary
// function as Biometric mutator.
type BiometricFunc func(context.Context, *model.BiometricMutation) (model.Value, error)

// Mutate calls f(ctx, m).
func (f BiometricFunc) Mutate(ctx context.Context, m model.Mutation) (model.Value, error) {
	if mv, ok := m.(*model.BiometricMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *model.BiometricMutation", m)
}
